...
ACTION ggoodsggoods::create(const name& issuer,
...
ACTION ggoodsggoods::issue(const name& to,
                     const name& category,
                     const name& token_name,
...
ACTION ggoodsggoods::burnnft(const name& owner,
...
ACTION ggoodsggoods::burnft(const name& owner,
...
ACTION ggoodsggoods::transfernft(const name& from,
                           const name& to,
                           const vector<uint64_t>& dgood_ids,
                           const string& memo ) {
...
ACTION ggoodsggoods::transferft(const name& from,
                          const name& to,
                          const name& category,
                          const name& token_name,
                          const asset& quantity,
...
ACTION ggoodsggoods::listsalenft(const name& seller,
                           const vector<uint64_t>& dgood_ids,
...
ACTION ggoodsggoods::closesalenft(const name& seller,
...
ACTION ggoodsggoods::confirmsale(const name& newowner, 
                        const name& owner, 
                        const asset& quantity,
                        const uint64_t& batch_id) {
...
void ggoodsggoods::buynft(const name& from,
                    const name& to,
                    const asset& quantity,
                    const string& memo) {
...
ACTION ggoodsggoods::logcall(const uint64_t& dgood_id) {
    require_auth( get_self() );
}

ACTION ggoodsggoods::logsale(const vector<uint64_t>& dgood_ids, const name& seller, const name& buyer, const name& receiver) {
    require_auth( get_self() );
}

ACTION ggoodsggoods::freezemaxsup(const name& category, const name& token_name) {
    require_auth( get_self() );
...
map<name, asset> ggoodsggoods::_calcfees(vector<uint64_t> dgood_ids, asset ask_amount, name seller) {
...
void ggoodsggoods::_changeowner(const name& from, const name& to, const vector<uint64_t>& dgood_ids, const string& memo, const bool& istransfer) {
    check (dgood_ids.size() <= 20, "max batch size of 20");
    // loop through vector of dgood_ids, check token exists
    dgood_index dgood_table( get_self(), get_self().value );
    lock_index lock_table( get_self(), get_self().value );
...
void ggoodsggoods::_checkasset(const asset& amount, const bool& fungible) {
...
void ggoodsggoods::_mint(const name& to,
                   const name& issuer,
                   const name& category,
                   const name& token_name,
...
void ggoodsggoods::_add_balance(const name& owner, const name& ram_payer, const name& category, const name& token_name,
...
extern "C" {
    void apply (uint64_t receiver, uint64_t code, uint64_t action ) {
        ...
        if ( code == self ) {
            switch( action ) {
                EOSIO_DISPATCH_HELPER( ggoodsggoods, (setconfig)(create)(issue)(burnnft)(burnft)(transfernft)(transferft)(listsalenft)(closesalenft)(confirmsale)(logcall)(freezemaxsup) )
            }
        }

        else {
            if ( code == name("eosio.token").value && action == name("transfer").value ) {
                execute_action( name(receiver), name(code), &ggoodsggoods::buynft );
            }
        }
    }
}
